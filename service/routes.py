...
def list_products():
    ...
    if name:
        app.logger.info("Requesting by name")
        products = Product.find_by_name(name)
    elif category:
        app.logger.info("Requesting by category")
        enum_value = getattr(Category, category.upper())
        products = Product.find_by_category(enum_value)
    elif available:
        app.logger.info("Requesting by available")
        # create bool from string
        available_value = available.lower() in ["true", "yes", "1"]
        products = Product.find_by_availability(available_value)
    else:
        app.logger.info("Requesting all")
...
def update_product(id):
    """
    Update a Product
    This endpoint will update a Product and return its new state
    """
